...
# ---------------------------- LLM provider ----------------------------
# Intentionally absent. Every LLM call in this repo goes through the `llm-chain`
# package: `import llm_chain as L; L.call_llm(...)`. The former llm()/_gemini()/
# _gemini_search()/_anthropic()/_post_with_retry()/resolved_provider() lived here and
# were removed in the llm-chain migration -- their retry policy allowed ~14 seconds of
# total patience and had no cross-model fallback, which is how a model shedding 503s
# turned into a blank report. This module now owns email and state IO only.
